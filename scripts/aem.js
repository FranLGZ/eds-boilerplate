...
function decorateButtons(element) {
  element.querySelectorAll('a').forEach((a) => {
    ...
    // NEW: Check if the link has a specific "plain" class or
    // if the title starts with an underscore (an easy trick for authors)
    const isPlain = a.textContent.startsWith('_') || a.classList.contains('plain');

    if (a.href !== a.textContent && !isPlain) {
      // Add !isPlain here
    ...
    } else if (isPlain && a.textContent.startsWith('_')) {
      // Clean up the underscore so it doesn't show on the website
